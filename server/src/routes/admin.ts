...
router.put('/users/:userId', adminOnly, async (req, res) => {
  ...
  try {
    ...
    // FIX: Возвращаем успех и логируем
    console.log('✅ [Admin] User updated: ' + userId + ' (Pro: ' + is_pro + ', Role: ' + role + ')');
    res.json({ success: true });
  } catch (err: any) {
...
router.post('/users/:userId/devices', adminOnly, async (req, res) => {
  ...
  try {
    ...
    for (const s of servers) {
      ...
      } catch (e: any) { console.error('Failed on server ' + s.name + ': ' + e.message); }
    }

    if (configs.length === 0) throw new Error('Could not connect to any VPN servers');
