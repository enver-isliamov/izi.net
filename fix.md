...
- [x] [2026-09-25 18:35] **SYS/ADMIN-USER-UNIVERSAL-ASSIGN-001**: Реализация назначения универсальной подписки и Pro-статуса в панели администратора:
  - `server/src/routes/admin.ts`: В роут `PUT /api/admin/users/:userId` добавлена поддержка сохранения поля `universal_access` (разрешение универсальной мультипротокольной подписки конкретному пользователю независимо от глобальной политики).
  - `migrations/004_add_user_pro_universal.sql` и `migrations/000_full_schema.sql`: Добавлены поля `is_pro` и `universal_access` в таблицу `users`.
  - `server/src/routes/config.ts`: В эндпоинт `/api/subscription/universal-link-visible` внедрена полноценная проверка авторизации, проверка глобальной настройки `UNIVERSAL_LINK_STATUS` ('all', 'pro', 'none') и персонального флага пользователя `universal_access` / `is_pro` / `admin`.
  - `src/pages/Admin/Users.tsx`: В карточки пользователей на десктопе и смартфонах добавлены интерактивные бейджи и кнопки управления: «🌐 Универсал: Вкл / Авто» и «👑 Pro Аккаунт» с мгновенным сохранением изменений.
