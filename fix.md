...
19. **Телеграм Бот: Инлайн кнопки, Поддержка и управление:**
    - Проблема: Бот был неинтерактивным, требовалось удобное меню и возможность обратиться в поддержку/сменить пароль на лету.
    - **Решение:** 
      - Бот переведен на инлайн-клавиатуры (`showMainMenu`).
      - Добавлена функция смены пароля прямо через бота.
      - Разработан режим "Связаться с поддержкой": сообщения пересылаются на `TELEGRAM_ADMIN_ID`, а ответы администратора транслируются пользователю.
      - Имя бота подтягивается из окружения (`TELEGRAM_BOT_NAME`).
