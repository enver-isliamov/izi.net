...
## SYS-08: Дополнительные порты панели XUI (41758)

Статус: [x] ИСПРАВЛЕНО. В `docker-compose.yml` добавлены порты `41758` (кастомный порт панели пользователя) и `8443` (VIP-порты VPN). Ранее из-за их отсутствия в конфигурации Docker-compose, внебиржевое подключение к панели по адресу `https://194.50.94.28:41758/...` блокировалось сетевым уровнем Docker на хосте.
