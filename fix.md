...
- [x] [2026-06-12 14:53] **ADMIN-API-001**: Исправлены 404/500 в админке (фронтенд вызывал отсутствующие `GET /api/admin/settings`, `GET /api/admin/servers/health`, server CRUD, admin device/payments/system endpoints, а `PUT /api/admin/users/:id` падал на старых схемах без `users.is_pro` -> в `server/src/routes/admin.ts` добавлены совместимые маршруты, диагностика Enot/settings с env/db fallback и обновление пользователя только по реально переданным полям).
