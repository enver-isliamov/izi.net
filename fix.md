...
30. **Исправление копирования и Reality-ссылок (Май 2026):**
    - Проблема: `navigator.clipboard` не срабатывал в iframe. Reality ссылки вызывали таймаут в Hiddify.
    - **Решение:** 
      - Добавлен `copyToClipboard` в `utils.ts` с использованием невидимого `textarea` как запасного варианта.
      - В `server.ts` исправлена генерация ссылок: добавлено кодирование фрагмента письма, исправлен маппинг полей Reality (поддержка вложенности `settings`), убраны лишние слэши.
31. **Стабильность Админ-панели (Серверы и Пользователи):**
    - Проблема: Страница серверов и пользователей зависала или выдавала ошибку 502/Network Error (Connection Reset), если один из VPN-узлов был недоступен.
    - **Решение:** 
      - Внедрены жесткие таймауты (3-5 сек) на получение "живых" данных (онлайн, клиенты) из панелей 3x-ui.
      - В маршруте `move-server` добавлена предварительная проверка доступности целевого сервера (7 сек) и проверка на попытку переноса на тот же сервер.
      - На фронтенде (`Admin/Users.tsx`) запросы к пользователям и серверам разделены на независимые блоки. Теперь сбой в загрузке статистики не блокирует отображение таблицы пользователей.
      - Добавлено пошаговое логирование в терминал с замером времени выполнения (`startTime`) и уникальными `requestId`, что позволяет выявлять медленные заграничные серверы.
      - Исправлена генерация ссылок при миграции: теперь параметры Reality (pbk, sid, sni) запрашиваются напрямую с целевого сервера, что исключает нерабочие конфиги.
      - Таймаут запросов к 3x-ui снижен до 5 сек для предотвращения 502 ошибок.
      - Пофикшено формирование ссылок подписки в Dashboard: теперь используется `VITE_API_URL` для корректной работы Hiddify/V2Box на внешних устройствах.
      - Добавлено сквозное логирование `[SubAPI][requestId]` для отладки проблем с обновлением профилей в приложениях.
