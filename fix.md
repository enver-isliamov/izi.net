...
28. **Исправление 405 Method Not Allowed на Vercel:**
    - Проблема: При попытке добавить сервер на развернутом сайте Vercel возвращал ошибку 405.
    - **Решение:** Перенастроен `vercel.json` с использованием билдера `@vercel/node` и корректных роутов для обработки POST/PUT запросов monolithic-сервером.
