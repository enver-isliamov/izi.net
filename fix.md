...
- [x] [2026-09-25 14:50] **SYS/TELEGRAM-AUTH-ROUTER-001**: Исправление привязки и авторизации Telegram, устранение ошибок 400 в профиле, приоритезация быстрого входа и разделение протоколов роутер (.conf) / приложения (ссылка):
  - `server/src/routes/user.ts`: Устранена ошибка `400 Bad Request` при обновлении настроек уведомлений в Профиле (теперь сервер принимает и синхронизирует любые имена ключей `subscription_expiry_alert` и `subscription_expiring`, с отказоустойчивым сохранением).
  - `server/src/services/bot.service.ts`: Исправлен обработчик `/start auth_...` и `/start link_...` в Telegram боте (устранено несоответствие префиксов токенов в базе, добавлено автоопределение имени бота через `getMe()` и мгновенная привязка `telegram_id` и `telegram_linked: true`).
  - `server/src/routes/user.ts`: Эндпоинт `/api/user/auth/telegram/verify` теперь поддерживает мгновенную 1-клик авторизацию и регистрацию для любого Telegram-пользователя без предварительной настройки пароля.
  - `src/pages/Profile.tsx`: Внедрён автоматический фоновый опрос статуса привязки Telegram — при подтверждении в боте профиль мгновенно переходит в статус «Привязан ✓» без перезагрузки страницы.
  - `src/pages/Login.tsx`: Быстрый вход в 1 клик через Google и Telegram вынесен на первое место в самый верх обеих форм (Вход и Регистрация) с яркими бейджами («В 1 клик», «Быстро»), а классический вход по Email размещен ниже.
  - `src/pages/Dashboard.tsx`: Переработан интерфейс подключения устройств: на каждой карточке устройства добавлены кнопки «⚡ Подключить», «📥 .conf» и «📋 Ссылка». Окно подключения разделено на 2 вкладки: «📱 Смартфон / ПК» (универсальная ссылка VLESS+Hysteria) и «📡 Роутер (.conf)» (готовый файл AmneziaWG/WireGuard с параметрами обфускации для Keenetic, OpenWrt, Mikrotik и AmneziaVPN).
