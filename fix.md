...
- [x] **SYS-01: Port 3005 Restoration** (Сервер возвращен на порт `3005` для совместимости с инфраструктурой OneDash и правилами Cloudflare Origin. API запросы переведены в относительный формат для корректной работы через прокси).
- [x] **SYS-02: Docker Port Standard** (Устранено расхождение портов в Dockerfile и docker-compose.yml. Теперь везде используется 3005:3005 для прозрачной работы).
- [x] **SYS-03: Cloudflare RF Patch** (Документировано решение с Nginx + DNS Only в RECO.MD для обхода блокировок Cloudflare в РФ и исправления ошибки 521).
