...
import { Server, Plus, Globe, Settings, Trash2, CheckCircle, XCircle } from 'lucide-react';
          ...
          >
            <form onSubmit={handleSubmit} className="p-6 bg-secondary/30 rounded-2xl border border-white/10 grid grid-cols-1 md:grid-cols-2 gap-4">
              <input
                ...
                className="bg-black/20 border border-white/5 rounded-xl p-3 text-sm"
                value={formData.name}
              ...
              <div className="md:col-span-2 flex gap-2">
                <button type="submit" className="px-6 py-2 bg-blue-600 rounded-xl text-sm font-medium">Сохранить</button>
                <button type="button" onClick={() => setIsAdding(false)} className="px-6 py-2 bg-white/5 rounded-xl text-sm">Отмена</button>
              </div>
            </form>
          </motion.div>
        )}
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground font-mono">{server.ip}{server.domain ? ` (${server.domain})` : ''}</p>
              </div>
            </div>
