...
async function run() {
  ...
  try {
    ...
    console.log("KEYS:", Object.keys(setRes.data.obj).join(', '));
    const xray = JSON.parse(setRes.data.obj.xrayTemplateConfig);
    console.log("API IN TEMPLATE:", !!xray.api);
    console.log("ALL SETTINGS DUMP:", JSON.stringify({
      apiPort: setRes.data.obj.apiPort,
      xrayTemplateConfig: "hidden for length",
      ...setRes.data.obj
    }, null, 2).slice(0, 500));
  } catch (e: any) {
